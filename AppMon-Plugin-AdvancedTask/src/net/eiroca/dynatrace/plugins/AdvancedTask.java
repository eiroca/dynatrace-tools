...
public class AdvancedTask extends AbstractTaskPlugin {
  ...
  static {
    AdvancedTask.tasks.put("WebService", WebServiceAction.class);
    AdvancedTask.tasks.put("SSH command", SSHCommandAction.class);
    AdvancedTask.tasks.put("Local command", LocalCommandAction.class);
  }
  ...
  public Status execute(final DynatraceContext<TaskEnvironment> context) throws Exception {
    ...
    final String taskType = context.getConfigString(AdvancedTask.CONFIG_TASKTYPE);
    final Class<?> taskClass = AdvancedTask.tasks.get(taskType);
    if (taskClass == null) {
